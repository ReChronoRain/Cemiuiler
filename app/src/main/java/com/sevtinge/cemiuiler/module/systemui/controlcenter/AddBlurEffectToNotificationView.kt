                ...
                override fun afterHookedMethod(param: MethodHookParam) {
                    ...
                    val mPanelViewController =
                        HookUtils.getValueByField(mController, "mPanelViewController")
                            ?: return
                    val isExpanding =
                        XposedHelpers.callMethod(mPanelViewController, if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.UPSIDE_DOWN_CAKE) "isExpandingOrCollapsing" else "isExpanding") as Boolean
                    if (isExpanding) {
                        return
                    }
                    val isOnKeyguard =
